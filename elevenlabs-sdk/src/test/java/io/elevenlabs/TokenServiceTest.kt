...
class TokenServiceTest {
    ...
    @Test
    fun `fetchPublicAgentToken returns token on success`() =
        runTest {
            server.enqueue(
                MockResponse()
                    .setResponseCode(200)
    ...
    @Test
    fun `fetchPublicAgentToken builds request URL with query parameters`() =
        runTest {
            server.enqueue(
    ...
    @Test
    fun `fetchPublicAgentToken omits environment when not provided`() =
        runTest {
            server.enqueue(
                MockResponse().setResponseCode(200).setBody("""{"token":"tok"}"""),
            )
    ...
    @Test
    fun `fetchPublicAgentToken throws on HTTP error with body in message`() =
        runTest {
            server.enqueue(
    ...
    @Test
    fun `fetchPublicAgentToken throws on malformed JSON`() =
        runTest {
            server.enqueue(
